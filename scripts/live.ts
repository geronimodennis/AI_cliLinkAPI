...
const filename = process.env.CLILINKAPI_CONFIG;
if (!filename) { console.error('BLOCKED: set CLILINKAPI_CONFIG to a private, configured JSON file with an official ChatGPT login.'); process.exitCode = 1; }
else {
  ...
  try {
    ...
    const workspace = process.env.CLILINKAPI_WORKSPACE ?? Object.keys(config.workspaces)[0]!;
    ...
    if (!response.ok) { console.error(JSON.stringify({ blocked: true, error: result.error })); process.exitCode = 1; }
      ...
      const nonce = randomUUID(); const localName = `.clilinkapi-live-${nonce}.txt`;
      const local = path.join(selected.path, localName); const outside = path.join(config.provider.codexHome, `.clilinkapi-live-${nonce}.txt`);
