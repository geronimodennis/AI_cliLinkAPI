...
export function createServer(config: Config, provider: Provider, log: (value: Record<string, unknown>) => void = terminalRequestLog, debugPayload = false) {
  ...
  const clientToolInstructions = (request: Generation['request']) => {
    ...
    return `Client-supplied functions registered for this request: ${names.join(', ')}. These functions execute on the requesting API client's computer, which owns the client's project workspace. For repository or coding tasks, first use the appropriate registered list, search, read, or shell function to discover the client's current workspace structure and relevant files. Treat those results as authoritative; do not infer the current tree from conversation history or inspect the gateway workspace instead. When a registered client function can perform the requested file, repository, search, edit, or shell operation, call it before attempting any provider-native or code-mode tool. Provider-native tools operate on the gateway computer and their host may be unavailable. Do not report that file or shell access is unavailable until you have attempted the appropriate registered client function.`;
  };
  const remoteInstructions = (workspace: string) => `This request is also connected to persistent remote workspace ${workspace}. For repository or coding tasks, first discover its current structure with remote_list_directory, remote_search_files, remote_read_file, or remote_shell_execute as appropriate; do not infer the tree from conversation history or the gateway workspace. For that workspace, use remote_read_file, remote_write_file, remote_list_directory, remote_search_files, remote_shell_execute, remote_git_status, remote_git_diff, and remote_git_log. Functions beginning with remote_ are executed by the connected remote worker. Provider-native filesystem and shell tools operate on the gateway computer, not the API client's project. Do not claim that tools are unavailable before attempting the appropriate registered client or remote_* function.`;
