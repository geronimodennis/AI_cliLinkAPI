...
export async function validatePaths(config: Config, filename: string): Promise<void> {
  ...
  for (const workspace of Object.values(config.workspaces)) {
    ...
    if (inside(workspace.path, configPath) || inside(workspace.path, config.provider.codexHome) || inside(config.provider.codexHome, workspace.path)) throw new Error('Keep clilinkapi configuration and Codex credential storage outside every workspace.');
...
export async function inspectWorkspace(workspace: Workspace, allowSymbolicLinks = true): Promise<string[]> {
  ...
  if (await realpath(workspace.path) !== workspace.path) throw new CliLinkAPIError(403, 'workspace_changed', 'Workspace identity changed. Restart after reviewing configuration.');
  const walk = async (directory: string, ancestors: Set<string>): Promise<void> => {
    const resolved = await realpath(directory);
    if (!inside(workspace.path, resolved) || ancestors.has(resolved)) throw new CliLinkAPIError(403, 'workspace_link', 'Workspace links must stay inside the workspace and must not form directory cycles.');
    const nextAncestors = new Set(ancestors).add(resolved);
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      const filename = path.join(directory, entry.name);
      let info = await lstat(filename);
      if (info.isSymbolicLink()) {
        if (!allowSymbolicLinks) throw new CliLinkAPIError(403, 'workspace_link', 'Symbolic links and junctions are disabled by provider.allowSymbolicLinks.');
        let target: string;
        try { target = await realpath(filename); }
        catch { throw new CliLinkAPIError(403, 'workspace_link', 'Workspace symbolic links must resolve to an existing target without link loops.'); }
        if (!inside(workspace.path, target)) throw new CliLinkAPIError(403, 'workspace_link', 'Symbolic links and junctions must point inside the same workspace.');
        info = await stat(filename);
      }
      if (!info.isDirectory() && info.nlink > 1) throw new CliLinkAPIError(403, 'workspace_link', 'Hard-linked files are not supported in workspaces.');
