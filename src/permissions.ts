...
const readAcl = `$ErrorActionPreference='Stop'; $p=$env:AICLITOAIAPI_ACL_PATH; $dir=[System.IO.Directory]::Exists($p); $a=if($dir){[System.IO.Directory]::GetAccessControl($p)}else{[System.IO.File]::GetAccessControl($p)};`;
...
async function windows(script: string, filename: string): Promise<void> { await exec('powershell.exe', ['-NoLogo', '-NoProfile', '-NonInteractive', '-Command', script], { env: { ...process.env, AICLITOAIAPI_ACL_PATH: filename }, windowsHide: true, timeout: 10000 }); }
