...
export function requireNativePlatform(allowUnqualifiedWindowsExecution = false, platform: NodeJS.Platform = process.platform): void {
  ...
  if (!['linux', 'darwin'].includes(platform)) throw new CliLinkAPIError(503, 'native_isolation_unavailable', 'No qualified native sandbox for this platform.');
}
export async function probeIsolation(rpc: Rpc, workspace: Workspace, home: string, signal: AbortSignal, allowUnqualifiedWindowsExecution = false): Promise<void> {
  ...
  try {
    ...
    const result = await rpc.request('command/exec', { command: [process.execPath, '-e', probe, local, outside, escaped, output, workspace.access, nonce], cwd: workspace.path, permissionProfile: 'clilinkapi', timeoutMs: 15000, outputBytesCap: 4096 }, signal) as { exitCode?: number; stdout?: string };
  ...
  } catch { throw new CliLinkAPIError(503, 'native_isolation_unavailable', 'Native sandbox read, write, symlink or subprocess isolation probe failed. Install native sandbox prerequisites; no agent turn was started.'); }
