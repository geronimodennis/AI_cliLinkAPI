...
export class Rpc {
  ...
  constructor(home: string, extraArgs: string[] = []) {
    ...
    this.child.stdout.on('data', (chunk: string) => {
      this.buffer += chunk;
      if (Buffer.byteLength(this.buffer) > 4 * 1024 * 1024) return this.fail(new CliLinkAPIError(502, 'upstream_overflow', 'Codex output exceeded protocol limits.'));
      let end: number;
      while ((end = this.buffer.indexOf('\n')) >= 0) {
        const line = this.buffer.slice(0, end); this.buffer = this.buffer.slice(end + 1);
        try { this.receive(JSON.parse(line)); } catch { this.fail(new CliLinkAPIError(502, 'upstream_protocol', 'Malformed Codex protocol output.')); }
      }
    });
    ...
    this.child.on('exit', () => { if (!this.closing) this.fail(new CliLinkAPIError(502, 'upstream_closed', 'Codex exited before completing the request.')); });
  }
  ...
  private receive(value: unknown) {
    ...
    if ('id' in message && typeof message.method === 'string') {
      ...
      if (this.queue.length >= 256) return this.fail(new CliLinkAPIError(502, 'upstream_overflow', 'Codex event queue exceeded its limit.'));
  ...
  async request(method: string, params: unknown, signal: AbortSignal): Promise<unknown> {
    ...
    const abort = () => { const p = this.pending.get(id); if (p) { clearTimeout(p.timer); this.pending.delete(id); p.reject(new CliLinkAPIError(499, 'cancelled', 'Request cancelled.')); } };
