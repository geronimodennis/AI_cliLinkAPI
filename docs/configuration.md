...
Capabilities are per workspace and control the native workspace tools available to both Codex and Antigravity. Native file read, file write, and shell tools are enabled when `capabilities` is omitted; the provider sandbox defaults off in that case. Explicit capability values still override these defaults. `fileRead` permits workspace file reads, `fileWrite` permits writes in a `read-write` workspace, `shell` enables shell tools and auto-approval, and `sandbox` enables the provider sandbox. Shell access is independent from file write access:
...
`X-Remote-Workspace-ID` defaults to the agent ID when omitted and must equal the worker's `workspaceId`. `X-Workspace-ID` is still required because it selects the gateway-side provider runner, native workspace, and concurrency lock. Provider-native file and shell tools remain enabled in remote mode, so the model can use either the gateway workspace or the connected remote workspace. Add `X-Remote-Agent-ID` to `GET /v1/models` to see `workspace_remote`, `remote_builtin_tools`, and `remote_custom_tools` reported as enabled.

Remote mode publishes every built-in worker tool under a collision-free remote namespace: `remote_read_file`, `remote_write_file`, `remote_list_directory`, `remote_search_files`, `remote_shell_execute`, `remote_git_status`, `remote_git_diff`, and `remote_git_log`. Provider-native tools remain available for the gateway workspace at the same time. Client-supplied OpenAI function definitions keep their original names and are returned to clients such as OpenCode for execution through the normal `tool_calls` response and `messages` continuation. A client-defined function is sent to the remote worker only when its name starts with `remote_`; it executes there only when the same exact name appears in the worker's `handlers`. The eight built-in names are reserved. Custom handlers receive the function argument object as JSON on stdin and return stdout as the tool result; no arbitrary client-supplied command path is executed.

When a request includes client tools, the gateway adds routing instructions that explicitly name them and tells the model to prefer those functions for client-side repository, file, edit, search, and shell work. This matters when OpenCode runs on a different computer: OpenCode's functions execute beside its project, while provider-native tools execute on the gateway computer and may have no code-mode host. A model must try the applicable registered client function before reporting that file or shell access is unavailable.

The bundled Codex app-server is launched with its code-mode host enabled because that host is required to dispatch registered dynamic functions back to clients such as OpenCode. The gateway still returns those calls through the OpenAI-compatible `tool_calls` response; it does not execute ordinary client functions on the gateway.
